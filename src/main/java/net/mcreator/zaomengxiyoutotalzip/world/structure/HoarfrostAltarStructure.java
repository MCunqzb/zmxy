...
public class HoarfrostAltarStructure {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
			feature = new Feature<NoFeatureConfig>(NoFeatureConfig.field_236558_a_) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if (dimensionType == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("zaomengxiyou:bonerealm")))
					...
					if ((random.nextInt(1000000) + 1) <= 150000) {
						...
						for (int a = 0; a < count; a++) {
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("zaomengxiyou", "hoarfrost_altar"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("hoarfrost_altar"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("zaomengxiyou:hoarfrost_altar"), configuredFeature);
		}
	}
	@SubscribeEvent
