...
public class WindParticle {
	...
	public static void registerParticleType(RegistryEvent.Register<ParticleType<?>> event) {
		event.getRegistry().register(particle.setRegistryName("wind"));
	}
	...
	private static class CustomParticle extends SpriteTexturedParticle {
		...
		protected CustomParticle(ClientWorld world, double x, double y, double z, double vx, double vy, double vz, IAnimatedSprite spriteSet) {
			...
			this.setSize((float) 4, (float) 4);
			this.particleScale *= (float) 8;
			this.maxAge = (int) Math.max(1, 200 + (this.rand.nextInt(12) - 6));
		...
		public void tick() {
			super.tick();
			if (!this.isExpired) {
				this.setSprite(this.spriteSet.get((this.age / 2) % 40 + 1, 40));
			}
		}
	}
