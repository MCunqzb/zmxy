...
package net.mcreator.zaomengxiyoutotalzip.particle;
...
public class FirebirdParticle {
	...
	private static class CustomParticle extends SpriteTexturedParticle {
		...
		protected CustomParticle(ClientWorld world, double x, double y, double z, double vx, double vy, double vz, IAnimatedSprite spriteSet) {
			...
			this.setSize((float) 0.5, (float) 1);
			this.particleScale *= (float) 8;
			this.maxAge = (int) Math.max(1, 50 + (this.rand.nextInt(6) - 3));
		...
		public void tick() {
			super.tick();
			if (!this.isExpired) {
				this.setSprite(this.spriteSet.get((this.age / 3) % 11 + 1, 11));
			}
		}
	}
